...
<center>
    ...
    <h5 style="color: blue;"> di Sistem informasi Pengadaan Barang dan Jasa YIARI</h5>
</center>
...
<script>
    ...
    new Chart(ctx, {
        type: 'bar',
        data: {
            labels: ['Total Divisi', 'Total Pegawai', 'Total Admin', 'TotalPengajuan', 'Pengajuan Diproses', 'Pengajuan Disetujui', 'Pengajuan Ditolak'],
            datasets: [{
                label: 'Grafik Data',
                data: ['{{$total_divisi}}', '{{$total_pegawai}}', '{{$total_admin}}', '{{$total_pengajuan}}', '{{$diproses}}', '{{$diterima}}', '{{$ditolak}}'],
                borderWidth: 1
            }]
        },
        options: {
